...
3. **`cw prc`** — Creates PR(s) to stg/main/both with title `[TICKET-ID] Title` (ticket from branch name).

4. **`cw review`** — Moves ticket to "Ready for QA".

5. **`cw cleanup`** — Triggers Cleanup Stale FE Namespaces (delete ephemeral namespace). Uses current branch by default; `-b` to specify branch.
